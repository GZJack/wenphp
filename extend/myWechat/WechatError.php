...
class WechatError
{
    private static $wechatErrorArray = array(
        "-1" => "系统繁忙，此时请开发者稍候再试",
        "0" => "请求成功",
    ...
    public static function get($key)
    {
        $key = (string) $key;
        dump(gettype($key));
        // 如果$key为字符串,就找对应的错误,否则就是整个错误对象,一个三目运算搞定
        return is_string($key) ? (isset(self::$wechatErrorArray[$key]) ? self::$wechatErrorArray[$key] : '未知错误码:'.$key) : self::$wechatErrorArray;
    }
}